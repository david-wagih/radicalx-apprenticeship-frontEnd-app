...
import ReqSkillsIcon from '../../../components/AddRoleForm/ReqSkillsIcon/ReqSkillsIcon';
import CheckBoxDropDown from '../../../components/AddRoleForm/CheckBoxDropDown/CheckBoxDropDown';
interface RequiredSkillsProps {
    reqSkills: string[];
}

const RequiredSkills: FC<RequiredSkillsProps> = ({ reqSkills }) => {
    ...
    return (
        <div className="req-skills-div flex flex-col items-start p-0 h-fit flex-none  flex-grow-0 gap-[8px] w-[555px]">
            ...
            <div className="req-skills box-border flex flex-row items-start h-12 bg-white flex-none  self-stretch flex-grow-0 py-[12px] pr-[12px] pl-[16px] gap-[10px] w-[555px] border-[1px]  border-[#CECECE] border-solid rounded-[12px]">
                <ReqSkillsIcon />
            ...
            {chosenSkills.length > 0 && (
                <div className="selected-skills flex flex-row items-start p-0 w-48 h-10 flex-none order-1 flex-grow-0 gap-[8px]">
                    {chosenSkills.map(skill => {
                        return (
                            <div
                                className="flex flex-row items-center w-fit h-10 flex-none order-none flex-grow-0 py-[8px] px-[12px] gap-[10px] rounded-[12px] bg-[#665fef33]"
